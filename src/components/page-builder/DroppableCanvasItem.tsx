...
export const DroppableCanvasItem: React.FC<DroppableCanvasItemProps> = ({ id, children, onDelete, onSelect }) => {
  const { setNodeRef, isOver } = useDroppable({
    ...
    data: { // Add data to identify this as a canvas item target
      isCanvasItem: true,
      componentId: id,
      accepts: ['container', 'split', 'form', 'table', 'text', 'button', 'image', 'dataCard', 'leadTable', 'inventoryTable', 'procurementTable', 'myRequestTable', 'inventoryRequestForm', 'procurementRequestForm', 'collapseCard','leadCarousel','oeLeadsTable','progressBar','leadProgressBar','cseProgressBar','ticketTable','ticketCarousel','ticketBarGraph','barGraph','lineChart','stackedBarChart','temporaryLogout','addUser','leadAssignment','openModalButton','jobManager','jobsPage','applicantTable','fileUpload','dynamicScoring','header','teamDashboard','analyticsBoard','zohoMail'], // Accept all component types
    },
  });
