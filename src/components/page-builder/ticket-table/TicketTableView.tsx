...
export function TicketTableView(props: TicketTableModel) {
  ...
  // DYNAMIC TITLE LOGIC: Intelligently infer the title from the active sidebar item
  // This overrides incorrect config.title values passed by the builder.
  const [mobileTitle, setMobileTitle] = React.useState(config?.title || "Support Tickets");
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (activeSidebarText.includes('wip') || pageTitle.includes('wip')) {
        setMobileTitle('WIP Tickets');
      } else if (activeSidebarText.includes('all support') || pageTitle.includes('all support')) {
        setMobileTitle('All Support Tickets');
      } else if (activeSidebarText.includes('pending') || pageTitle.includes('pending')) {
        setMobileTitle('Pending Tickets');
      } else if (activeSidebarText.includes('not connected') || pageTitle.includes('not connected')) {
        setMobileTitle('Not Connected Tickets');
      } else {
        setMobileTitle(config?.title || "Support Tickets");
      }
    }
  }, [config?.title]);
  ...
  return (
    <>
      {/* Mobile Page Title - Dynamically changes based on active page context */}
      ...
      <div className="font-body overflow-x-auto border-2 border-gray-200 rounded-lg bg-white p-4">
        ...
        <div className="mb-4 relative">
          {/* Toolbar - Internal title removed for desktop to prevent duplication, elements aligned right */}
          <div className="flex items-center gap-3 flex-wrap justify-end w-full mb-4">
