...
const BillingPage = () => {
  ...
  const {
    ...
    error: queryError,
  } = useBillingReport(month, roleRates, Boolean(month) && !selectedIsFuture);

  const billingMutation = useBillingReportMutation();

  const report: BillingReport | null = queryReport ?? null;
  const isLoading =
    isQueryLoading || isQueryFetching || billingMutation.isPending;
  ...
  }, [report?.month, report?.period_start, report?.period_end]);

  useEffect(() => {
    if (!queryError || billingMutation.isPending) return;
    const message =
      queryError instanceof Error
        ? queryError.message
        : 'Failed to load billing report';
    toast.error(message);
  }, [billingMutation.isPending, queryError]);

  useEffect(() => {
    if (!billingMutation.error) return;
    const err = billingMutation.error;
    const message =
      err instanceof Error ? err.message : 'Failed to load billing report';
    toast.error(message);
