/**
 * Pyro Jobs API Service
 * Manual enqueue / status / re-run for Brahma/Vishnu pyro_job rows.
 */
...
export interface PyroJobDetail {
  id?: number;
  job_name?: string;
  status?: string;
...
export function getPyroJobPayloadTemplate(jobName: string): Record<string, unknown> {
  return PYRO_JOB_PAYLOAD_TEMPLATES[jobName]
    ? structuredClone(PYRO_JOB_PAYLOAD_TEMPLATES[jobName])
    : {};
}
...
export async function getPyroJobStatus(
  jobId: string | number,
): Promise<PyroJobDetail> {
  const response = await apiClient.get(`/pyro-jobs/${jobId}/`);
  return response.data;
}

export async function rerunPyroJob(
  jobId: string | number,
): Promise<ManualPyroJobRunResult> {
...
export const pyroJobsService = {
  listTypes: listRunnablePyroJobTypes,
  getPayloadTemplate: getPyroJobPayloadTemplate,
  enqueue: enqueuePyroJob,
  getStatus: getPyroJobStatus,
  rerun: rerunPyroJob,
};
