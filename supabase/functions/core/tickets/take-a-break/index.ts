...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: ticket, error: ticketError } = await supabase.from('support_ticket').select('id, resolution_status, assigned_to, cse_name').eq('id', ticketId).single();
    ...
    if (shouldUnassign) {
      // Unassign the ticket
      const { error: updateError } = await supabase.from('support_ticket').update({
        assigned_to: null,
        cse_name: null
      }).eq('id', ticketId);
      if (updateError) {
